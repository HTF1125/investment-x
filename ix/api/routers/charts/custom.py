...
from ix.api.dependencies import get_current_user, get_optional_user, user_role as _user_role, is_owner_role as _is_owner, is_admin_role as _is_admin, user_id_str as _user_id
...
def generate_pdf_buffer(
        ...
        try:
            ...
            for future in concurrent.futures.as_completed(future_to_index, timeout=600):
                ...
                try:
                    ...
                    if data:
                        logger.info(
                            "Successfully rendered png for index %d: %s", index, chart_name
                        )
                    else:
                        logger.warning(
                            "Render returned None for index %d: %s", index, chart_name
                        )
                    results[index] = data
                    if progress_cb:
                        completed_renders += 1
                    ...
                    logger.error(
                        "Parallel rendering failed for chart at index %d (%s): %s", index, chart_name, e
                    )
                    if progress_cb:
                        completed_renders += 1
                        progress_cb(
    ...
    for i, img_data in enumerate(results):
        if img_data is None:
            ...
            logger.warning("PDF: chart '%s' rendered as None, substituting placeholder", chart_name)
...
def preview_custom_chart(
    ...
    try:
        ...
        logger.info(
            ...
            exec_duration, serial_duration, total_duration, len(json_str),
        )
...
def create_custom_chart(
    ...
    logger.info("New chart created with ID: %s, Name: '%s'", new_chart.id, new_chart.name)
...
def export_custom_charts_pdf(
    ...
    _update_process = update_process
...
def refresh_custom_chart(
    ...
    try:
        ...
        logger.debug("Refresh traceback for chart %s:\n%s", chart_id, traceback.format_exc())
        if isinstance(e, HTTPException):
            raise e
        raise HTTPException(
            status_code=400,
...
def update_custom_chart(
    ...
    logger.info(
        "PUT /custom/%s - New Code Len: %s, Old Code Len: %s", chart_id, code_len, old_code_len
    )

    # Update fields if provided
    if update_data.name is not None:
        logger.info("Setting chart name from '%s' to '%s'", chart.name, update_data.name)
    ...
    if update_data.code is not None:
        ...
        try:
            ...
            logger.info("Generated new figure for %s. Snippet: %s...", chart_id, fig_str)
        except Exception as e:
            logger.error("Failed to update figure for chart %s: %s", chart_id, e)
            logger.debug("Update traceback for chart %s:\n%s", chart_id, traceback.format_exc())
            if isinstance(e, HTTPException):
                raise e
            raise HTTPException(
                status_code=400,
    ...
    logger.info(
        "Chart %s updated successfully. Figure updated: %s", chart.id, update_data.code is not None
    )
    return _to_custom_chart_response(chart)
