...
try:
    ...
    from ix.web.app import server as dash_server
    ...
    # Note: We mount it at the root, so it handles everything not handled by the API routers above.
    # API routers are prefixed with /api, so they take precedence if matched first?
    # FastAPI matches in order. Since mounted app matches a path prefix, and "/" matches everything,
    # we should check if this shadows /api.
    # Actually, specific routes (like app.include_router) usually take precedence over mounted apps if defined first.
    # But to be safe, we usually mount WSGI apps strictly or rely on path matching order.
    # However, mounting at "/" might capture everything.
    # Better strategy: API routes are defined above.
    # FastAPI documentation says: "The first one that matches is used."
    # So if we define API routes first, they are checked first.

    logger.info("Mounting Dash app...")
    app.mount("/", WsgiToAsgi(dash_server))
