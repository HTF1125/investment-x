...
class ChartTheme:
    ...
    def _apply_datetime_padding(self, fig: go.Figure) -> None:
        ...
        for axis_name in self._axis_names(fig, "xaxis"):
            ...
            # Skip axes that already have an explicit range set (numeric or
            # date strings like "2020-01-01").  These were intentionally set
            # by buildChartFigure (startDate/endDate) or user config.
            axis_range = getattr(axis_obj, "range", None)
            if axis_range is not None and len(axis_range) >= 2:
                # If autorange is explicitly False, the range was intentional
                if getattr(axis_obj, "autorange", None) is False:
                    continue
                try:
                    ...
                    # Date-string ranges (e.g. "2020-01-01") aren't numeric
                    # but are still intentional — preserve them.
                    if all(isinstance(v, str) and len(v) >= 8 for v in axis_range[:2]):
                        continue
