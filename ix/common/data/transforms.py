...
def clean_series(series: pd.Series, name: str | None = None) -> pd.Series:
    """Coerce to numeric, drop NaN/inf, sort by index. Canonical implementation."""
    clean = pd.to_numeric(series, errors="coerce")
    clean = clean.replace([np.inf, -np.inf], np.nan).dropna()
    if clean.empty:
        return pd.Series(dtype=float, name=name) if name else clean
    try:
...
def Resample(
    ...
    Args:
        ffill: If True, forward-fill gaps and drop NaN after resampling.
    """
    result = series.resample(freq).last()
    if ffill:
...
def SimilarPatterns(
    ...
    """
    from ix.common.quantitative.pattern_search import find_similar_patterns
