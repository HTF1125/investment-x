...
def ecb_total_assets(freq: str = "W") -> pd.Series:
    ...
    """
    ecb_eur = Series("ECBASSETSW", freq=freq)  # FRED: ECB assets (EUR millions)
    if ecb_eur.empty:
...
def boj_total_assets(freq: str = "W") -> pd.Series:
    ...
    """
    boj_jpy = Series("JPNASSETS", freq=freq)  # FRED: BOJ assets (JPY 100M)
    if boj_jpy.empty:
...
def g4_balance_sheet() -> pd.DataFrame:
    """G4 central bank balance sheets in USD trillions.

    Fed + ECB + BOJ. PBoC excluded due to data availability.
...
def central_bank_liquidity_composite(window: int = 120) -> pd.Series:
    ...
    """
    from ix.db.custom.liquidity import fed_net_liquidity
