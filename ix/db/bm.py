...
def ConstantReturnBenchmark(annual_return: float = 0.01, start: str = "2000-1-1", end: str | None = None):
    if end is None:
    ...
    benchmark_series = pd.Series(data=100 * cumulative_returns, index=dates, name="Benchmark")
    return benchmark_series

def MezzAlpha():
    d = ConstantReturnBenchmark(0.02).pct_change().mul(0.70)
    b = Series("KOSPI Index:PX_LAST").resample('D').last().ffill().pct_change().mul(0.30)
    x = (d + b).add(1).cumprod()
    x.name = "MezzAlpha"
    return x.dropna()
