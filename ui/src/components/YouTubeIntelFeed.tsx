...
import { ExternalLink, Youtube, AlertTriangle, Save, Trash2, PlusCircle, FileText, RefreshCw, Play, Square, SlidersHorizontal } from 'lucide-react';
...
export default function YouTubeIntelFeed({ embedded }: { embedded?: boolean } = {}) {
  ...
  const [viewMode, setViewMode] = useState<'media' | 'reader'>('reader');
  ...
  useEffect(() => {
    ...
    const savedViewMode = localStorage.getItem('intel_youtube_view_mode');
    if (savedViewMode === 'media' || savedViewMode === 'reader') {
    ...
    <section className={outer}>
      <div className="h-10 flex items-center justify-between px-3 border-b border-border/60 shrink-0 relative">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs font-semibold text-foreground">YouTube Intelligence</span>
        </div>
        <div className="flex items-center gap-2 text-[10px] text-muted-foreground/60">
          ...
          <button
            onClick={() => {
              const next = viewMode === 'reader' ? 'media' : 'reader';
              setViewMode(next);
              if (typeof window !== 'undefined') localStorage.setItem('intel_youtube_view_mode', next);
            }}
            className="h-6 px-2 rounded-md border border-border/60 inline-flex items-center justify-center hover:bg-foreground/[0.06] text-[10px] text-muted-foreground"
            title="Toggle reader mode"
          >
            {viewMode === 'reader' ? 'Reader' : 'Media'}
          </button>
          <button
            onClick={() => setControlsOpen((v) => !v)}
            className="h-6 w-6 rounded-md border border-border/60 inline-flex items-center justify-center hover:bg-foreground/[0.06] text-muted-foreground"
        ...
        <div className="px-3 py-2 border-b border-border/60 space-y-2">
          <div className="grid grid-cols-1 md:grid-cols-[180px_1fr] gap-2">
            <select
              ...
              className="h-7 px-2.5 rounded-md border border-border/50 bg-transparent text-[11px] focus:outline-none transition-colors"
            >
              <option value="unsummarized">Unsummarized first</option>
            ...
            <input
              ...
              placeholder="Search title/channel/summary"
              className="h-7 px-2.5 rounded-md border border-border/50 bg-transparent text-[11px] focus:outline-none transition-colors"
            />
          </div>
          {isAdmin && (
            <div className="grid grid-cols-1 md:grid-cols-[1fr_auto] gap-2">
              <input
                ...
                className="h-7 px-2.5 rounded-md border border-border/50 bg-transparent text-[11px] focus:outline-none focus:border-border transition-colors"
              />
              <button
                onClick={() => {
                ...
                className="h-7 px-2.5 bg-foreground text-background rounded-md text-[10px] font-medium hover:opacity-80 disabled:opacity-50 transition-opacity inline-flex items-center justify-center gap-1.5"
              >
          ...
          <div className="text-[10px] text-muted-foreground/50">Minimum video length: 5 minutes.</div>
        </div>
      )}
      {adminError && (
        <div className="mx-3 mt-2 rounded-md border border-rose-500/30 bg-rose-500/[0.06] px-3 py-1.5 text-[11px] text-rose-400">
          {adminError}
        </div>
      )}
      {ttsError && (
        <div className="mx-3 mt-2 rounded-md border border-amber-500/30 bg-amber-500/[0.06] px-3 py-1.5 text-[11px] text-amber-400">
          {ttsError}
        </div>
      )}
      ...
      <div className="min-h-0 flex-1 overflow-y-auto custom-scrollbar divide-y divide-border/40">
        {videos.map((v) => {
          ...
          const collapseAt = 14;
          return (
            <article key={v.video_id} className="px-2.5 py-2 hover:bg-foreground/[0.02] transition-colors">
              <div className={`grid ${viewMode === 'media' ? 'grid-cols-1 md:grid-cols-[108px_1fr]' : 'grid-cols-1'} gap-2.5 items-start`}>
                {viewMode === 'media' && (
                  <a
                    href={v.url}
                    target="_blank"
                    rel="noreferrer"
                    className="group relative rounded-lg overflow-hidden border border-border/60 bg-background w-full max-w-full sm:max-w-[108px]"
                  >
                    <img
                      src={`https://i.ytimg.com/vi/${v.video_id}/hqdefault.jpg`}
                      alt={v.title}
                      className="w-full h-[70px] object-cover opacity-85 group-hover:opacity-100 transition-opacity"
                      loading="lazy"
                    />
                    <div className="absolute bottom-1.5 left-1.5 text-[9px] font-mono px-1 py-0.5 rounded bg-background/90 text-muted-foreground border border-border/60">
                      Watch
                    </div>
                  </a>
                )}
                ...
                <div className="min-w-0 space-y-1.5">
                  <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-2 sm:gap-3 mb-1.5">
                    <div className="min-w-0">
                      <a
                        href={v.url}
                        target="_blank"
                        rel="noreferrer"
                        className="text-[12px] font-semibold text-foreground hover:text-foreground/70 transition-colors inline-flex items-center gap-1"
                      >
                        <span className="break-words">{v.title}</span>
                        <ExternalLink className="w-3 h-3 shrink-0 text-muted-foreground/50" />
                      </a>
                      <div className="text-[9px] text-muted-foreground/60 font-mono mt-1 flex flex-wrap items-center gap-1.5">
                        <span>{v.channel}</span>
                    ...
                    {isAdmin ? (
                      <div className="space-y-2">
                        <textarea
                          ...
                          className="w-full min-h-[96px] px-2 py-1.5 rounded-md border border-border/50 bg-transparent text-[11px] focus:outline-none focus:border-border transition-colors"
                        />
                        <button
                          onClick={() =>
                          ...
                          <div className="space-y-3">
                            ...
                            <div className="space-y-2.5">
                              {(() => {
                                ...
                                if (!rows.length) {
                                  return (
                                    <span className="text-muted-foreground/50 text-[12px]">No summary yet.</span>
                                  );
                                }
                                return rows;
                              })()}
