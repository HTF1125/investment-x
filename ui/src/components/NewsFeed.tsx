...
export default function NewsFeed({ embedded }: { embedded?: boolean }) {
  ...
  if (isLoading) {
    return (
      <div className="h-32 border border-border/50 rounded-lg bg-background animate-pulse flex items-center justify-center text-muted-foreground text-sm">
  ...
  const outer = embedded
    ...
    : 'h-full border border-border/50 rounded-lg overflow-hidden bg-background flex flex-col min-h-0';

  return (
    <section className={outer}>
      <div className="px-3 pt-2.5 pb-2 border-b border-border/50 shrink-0 bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background sticky top-0 z-20">
        ...
        <div className="flex items-center gap-2 relative" ref={filterRef}>
          <button
            ...
            className={`flex items-center gap-1.5 h-7 px-2.5 rounded-md border text-[11px] font-medium transition-all ${
              filterOpen || !allSelected
                ...
                : 'border-border/50 bg-primary/[0.04] text-muted-foreground hover:text-foreground hover:bg-primary/10'
            }`}
          >
          ...
          {filterOpen && (
            <div className="absolute top-full left-0 mt-1.5 w-64 max-h-[80vh] overflow-y-auto p-1.5 rounded-lg border border-border bg-background shadow-lg z-50 custom-scrollbar animate-in fade-in zoom-in duration-150">
              <button
                onClick={toggleAll}
                className="w-full flex items-center justify-between px-2.5 py-1.5 rounded-lg text-[11px] font-medium text-foreground hover:bg-primary/10 transition-colors"
              >
              ...
              {allThemes.map((theme) => {
                ...
                return (
                  <button
                    ...
                    className="w-full flex items-center justify-between px-2.5 py-1.5 rounded-lg text-[11px] font-medium text-muted-foreground hover:text-foreground hover:bg-primary/10 transition-colors"
                  >
                    <span className={isSelected ? 'text-primary' : ''}>{theme}</span>
        ...
        <div className="px-2 py-2 grid grid-cols-1 md:grid-cols-2 gap-2">
          {rows.slice(0, 40).map((item) => {
            ...
            return (
              <article
                key={item.id}
                className="px-3 py-2.5 rounded-[var(--radius)] border border-border/40 hover:bg-primary/[0.04] transition-colors min-h-[112px]"
              >
                ...
                <div className="mb-1 flex flex-wrap gap-1">
                  {item.__themes.slice(0, 3).map((tag) => (
                    <span key={`${item.id}-${tag}`} className="text-[9px] px-1.5 py-0.5 rounded border border-border/50 text-muted-foreground/80">
                      {tag}
                    </span>
                  ))}
                </div>
                <a
                  ...
                  className="text-[12px] md:text-[13px] font-semibold text-foreground leading-snug hover:text-primary transition-colors"
                >
                  {item.title}
                </a>
                {item.summary && (
