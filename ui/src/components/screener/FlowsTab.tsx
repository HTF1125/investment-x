...
import type { FlowEntry, ConsensusEntry, SectorConcentration } from './types';

const ACTIONS = ['ALL', 'NEW', 'INCREASED', 'DECREASED', 'SOLD'] as const;

type FlowView = 'flows' | 'consensus';
...
export default function FlowsTab({
  flows, consensus, sectors,
  isLoading, consensusLoading, sectorLoading,
  ...
  const formStyle = { colorScheme: theme === 'light' ? 'light' : 'dark', backgroundColor: 'rgb(var(--background))', color: 'rgb(var(--foreground))' } as const;
    ...
    return (
      <div className="flex items-center justify-center py-10">
        <div className="flex flex-col items-center gap-2">
          ...
          <span className="text-[11.5px] text-muted-foreground/50 tracking-widest uppercase">Loading flows</span>
        </div>
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="panel-card p-6 text-center">
        <p className="text-[13px] text-destructive/80 mb-1">Failed to load flow data</p>
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex items-center gap-1 mb-1">
        {(['flows', 'consensus'] as FlowView[]).map(v => (
          <button
            key={v}
            onClick={() => setView(v)}
            className={`h-7 px-2.5 rounded-[var(--radius)] text-[11.5px] font-semibold uppercase tracking-[0.06em] transition-all ${
              view === v
                ? 'bg-foreground text-background'
                : 'bg-foreground/[0.04] text-muted-foreground hover:text-foreground hover:bg-foreground/[0.08]'
            }`}
          >
            {v === 'flows' ? 'All Flows' : 'Consensus'}
          </button>
        ))}
      </div>
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            <div className="relative min-w-[160px]">
              ...
              <input
                ...
                className="w-full h-7 pl-8 pr-3 border border-border/50 rounded-[var(--radius)] text-[12.5px] focus:outline-none focus:border-primary/50 focus:ring-1 focus:ring-primary/20"
                style={formStyle}
              />
            </div>
              ...
              <table className="w-full text-[13px]">
                <thead>
                  <tr className="border-b border-border/30">
                    {['Fund', 'Symbol', 'Action', 'Shares', 'Value', 'Change %', 'Q/Q %', 'Report Date', 'VOMO'].map(h => (
                ...
                <tbody>
                  {filtered.map((flow, i) => (
                    ...
                    >
                      ...
                      <td className={`px-2.5 py-1.5 font-semibold text-[11.5px] uppercase tracking-wider ${ACTION_COLORS[flow.action] || 'text-muted-foreground'}`}>
                        {flow.action}
                      </td>
                      ...
                      {/* Q-over-Q column */}
                      <td className={`px-2.5 py-1.5 font-mono tabular-nums ${
                        flow.qoq_shares_change_pct !== null
                          ? flow.qoq_shares_change_pct >= 0 ? 'text-success/80' : 'text-destructive/80'
                          : 'text-muted-foreground/30'
                      }`}>
                        {flow.qoq_shares_change_pct !== null ? `${fmtNum(flow.qoq_shares_change_pct)}%` : '-'}
                      </td>
                  ...
                  {filtered.length === 0 && (
                    <tr>
                      <td colSpan={9} className="px-4 py-8 text-center text-muted-foreground/40 text-[13px]">
      ...
      <SectorBreakdown sectors={sectors} isLoading={sectorLoading} />
    </div>
  );
}
