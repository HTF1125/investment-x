...
import { Radio, ChevronLeft, ChevronRight, Calendar, PanelRightOpen, PanelRightClose } from 'lucide-react';
...
export default function IntelHeader({ state }: IntelHeaderProps) {
  ...
  const showSidePanelToggle = activeTab !== 'wartime' && activeTab !== 'stress';
  ...
  return (
    <div className="h-9 px-4 sm:px-5 lg:px-6 border-b border-border/25 flex items-center justify-between shrink-0 bg-background">
      ...
      {/* Right: Date nav + side panel toggle */}
      <div className="flex items-center gap-3">
        {/* Date navigator (Research tab only) */}
        {showDateNav && (
        ...
        {showSidePanelToggle && (
          <button
            ...
            title={sidePanelOpen ? 'Close feeds panel' : 'Open feeds panel'}
          >
            {sidePanelOpen ? (
